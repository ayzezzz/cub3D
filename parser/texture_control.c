...
/*   Updated: 2025/02/05 19:01:51 by itulgar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if((clean_str[0] == 'N' && clean_str[1] == 'O' && clean_str[2] == ' ') && (data->textures.check_list[NO]+= 1)){
        if(data->textures.check_list[NO] == 1)
            data->textures.no  = find_texture_path(clean_str + 3);
    }
        else if ((clean_str[0] == 'S' && clean_str[1] == 'O' && clean_str[2] == ' ') && (data->textures.check_list[SO]+= 1)){
                if(data->textures.check_list[SO] == 1)
            data->textures.so  = find_texture_path(clean_str + 3);
        }
        else if((clean_str[0] == 'W' && clean_str[1] == 'E' && clean_str[2] == ' ') && (data->textures.check_list[WE]+= 1)){
               if(data->textures.check_list[WE] == 1)
            data->textures.we  = find_texture_path(clean_str + 3);   
        }
        else if ((clean_str[0] == 'E' && clean_str[1] == 'A' && clean_str[2] == ' ') && (data->textures.check_list[EA]+= 1)){
