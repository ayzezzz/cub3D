...
/*   Updated: 2025/02/05 20:01:25 by zayaz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (side == 0)
		data->rayc->hit_x = data->player.p_y + data->rayc->wall_dist
			* data->rayc->ray_dir_y;
	else
		data->rayc->hit_x = data->player.p_x + data->rayc->wall_dist
			* data->rayc->ray_dir_x;
	data->rayc->hit_x -= floor(data->rayc->hit_x);
	data->rayc->text_x = (int)(data->rayc->hit_x * 64);
	data->rayc->per_pix = 1.0 * 64 / line_h;
	data->rayc->tex_y_next = (data->rayc->wall_start - S_HEIGHT / 2 + line_h
			/ 2) * data->rayc->per_pix;
}
...
void	draw_textured_column(t_data *data, int side, int col)
{
	...
	{
		data->rayc->text_y = (int)data->rayc->tex_y_next
			& (data->cub_mlx->map_size - 1);
		data->rayc->tex_y_next += data->rayc->per_pix;
		text_i = data->rayc->text_x + data->cub_mlx->map_size
			* data->rayc->text_y;
