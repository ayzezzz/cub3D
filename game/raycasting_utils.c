...
/*   Updated: 2025/01/31 19:44:56 by itulgar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if(side == 0)
        data->rayc->hit_x = data->player.p_x + data->rayc->wall_dist * data->rayc->ray_dir_y;
    else
        data->rayc->hit_x = data->player.p_y + data->rayc->wall_dist * data->rayc->ray_dir_x;
    data->rayc->hit_x -= floor(data->rayc->hit_x);
    data->rayc->text_x = (int)(data->rayc->hit_x * 64);
    data->rayc->text_x = 64 - data->rayc->text_x - 1;
...
void draw_textured_column(t_data *data,int side,int col)
{
  ...
  //  int text_i;
    ...
    {
        data->rayc->text_y = (int)data->rayc->tex_y_next & 63;
        data->rayc->tex_y_next += data->rayc->per_pix;
      //  text_i = data->rayc->text_x + 64 * data->rayc->text_y;
        if(data->rayc->ray_dir_x > 0 && side != 1)
            color = data->cub_mlx->te_img[SO]->addr[ (64 * data->rayc->text_y)  \
			+ data->rayc->text_x];
        else if(data->rayc->ray_dir_x < 0 && side != 1)
            color = data->cub_mlx->te_img[NO]->addr[ (64 * data->rayc->text_y)  \
			+ data->rayc->text_x];
        else   if(data->rayc->ray_dir_x <= 2 && data->rayc->ray_dir_y >= 0 && side == 1)
            color = data->cub_mlx->te_img[EA]->addr[ (64 * data->rayc->text_y)  \
			+ data->rayc->text_x];
        else
            color = data->cub_mlx->te_img[WE]->addr[(64 * data->rayc->text_y)  \
			+ data->rayc->text_x];
